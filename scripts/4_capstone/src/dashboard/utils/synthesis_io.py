...
BAD_KEYWORDS = ["method", "flag", "qc", "unit", "note", "comment", "id", "type", "carbonate", "carbonates", "inorganic"]
...
def score_column(
    name: str, keywords: list[str], bad_keywords: list[str] | None = None, preferred: list[str] | None = None
) -> float:
...
def find_best_column(
    ...
    if keywords:
        scored = [(c, score_column(c, keywords, bad_keywords=bad_keywords, preferred=preferred)) for c in columns]
...
def find_candidate_studies(df_inv: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in derivative.iterrows():
        ...
        if som_col and bd_col:
            records.append({"path": row["path"], "study_id": row["study_id"], "som_col": som_col, "bd_col": bd_col})
